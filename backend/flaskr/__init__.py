...
def create_app(test_config=None):
    ...
    def get():
        ...
        return jsonify({"questions":current_q,"total_questions":len(Question.query.all()),"categories":dict_c,"current_category":f_c['type']})
    ...
    def add():
        ...
        cat=Question(question=n_question,answer=n_answer,difficulty=n_difficulty,category=n_category)
        cat.insert()
    ...
    def search():
        ...
        all_q=Question.query.filter(Question.question.ilike(f'%{term}%')).all()
        ...
        return jsonify({"questions":formated_q,"totalQuestions":len(Question.query.all()),"currentCategory":current_c.format()['type']})
    ...
    def quizzes():
        ...
        cat=Category.query.filter(Category.type == quiz_category['type']).one_or_none()

        if cat is None:
            abort(404)

        question=Question.query.filter(Question.category == cat.id, Question.id.notin_(previous_questions)).order_by(Question.id).first()
